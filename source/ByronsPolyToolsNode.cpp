...
//----------------------------------------------------------------------------------------------------------
{
	...
	/*
	MPRINT("-------------------------------")
	MPRINT("Connection Wird gelÃ¶scht")
...
*/
	...
	// Mal die softTransformationEngine prozedur aufrufen - sie kümmert sich um die visNode, welche ansonsten verhindern würde, dass die history wirklich gekillt wird
...
//	FnFloatAttr.setMax(1.0);
	...
	// Zuletzt die SoftTransformationAttribute hinzufügen
	// Per Macro - dirty, aber funktioniert - wie machen die ALIAS Typen das ??? Die leiten auch ständig von einer BaseNode ab, und da gehen dann keine Attribute flöten
	// Oder werden unbrauchbar so wie bei mir, so dass im Endeffekt sämtliche Attribute ein eindeutiges statisches Attribut haben müssen
...
//	cout<<"War hier, plug war "<<plug.info()<<endl;
	...
	// Sich jetzt um die STE internals kümmern
	//
	//
...
//---------------------------------------------
{
	...
	if(meshPath.apiType() != MFn::kInvalid && nodePtr->validIndices.length() != 0)	//zur Sicherheit, sollte aber eigentlich nicht mÃ¶glich sein
	{
...
//----------------------------------------------------------------------------
{
				...
				//gleich zu beginn muss der MeshPath initialisiert werden, damit der MeshPath an die fty Ã¼bergeben werden kann
				...
				// Diese Methode ist essentiell, da sie zugriff auf alle Attribute ermöglicht
			...
			//	MPxNode::setExistWithoutInConnections(true);
				...
				// **************************************************************************************
				// JETZT DIE INPUT DATEN SETZEN FÜR SOFTTRANSFORMATION NODE
				// **************************************************************************************
				...
				case 5: // SC
					{
						// Hier ist es aufwendig: Alle neuen Vtx und jene, die im inMesh die gewählten kanten ausgemacht haben müssen gewählt werden
				...
				//jetzt die Components anwÃ¤hlen
					...
					if( meshPath.isValid() )
					{
						...
						if(optionsArray[6] != 5)
						{
							if(mode == 0)
							{
								...
								//wenn nÃ¶tig edges in Faces umwandeln
								if(optionsArray[6] == 2)
								{
							...
							else if(mode == 4)
							{
								...
								{
									//fty NH erstellen fÃ¼r external Boundary
									BPTfty_NH fty_NH;
...
//	FactoryWerte setzen
//	(hier Ã¼berall eventuell noch MCheck nutzen fÃ¼r Debug wenn nÃ¶tig)
	MStatus status;
	...
	//	INVIS(cout<<"Plug in BPTCompute ist: "<<plug.name()<<endl);		
		...
		else
		{
			if( (plug == BPToutMesh) )
			{
				...
				// -----------------------------------------------------------------
				// Erstmal sämtliche Parameter holen, die den output beeinflussen
				// -----------------------------------------------------------------
				...
				if(meshDirty)
				{
					...
					//jetzt noch die SoftTransformationEngine mit daten bestücken
					//------------------------------------------------------------
					softTransformationEngine::extractStaticData(plug, data);


					// DEAKTIVIEREN DER VISUALISIERUNG,  WENN DIE NODE ABGEWÄHLT WIRD - 
				...
				{
					// An dieser stelle ist es gut möglich, dass die history geändert wurde
					...
					if(cp == op)
					{
						...
						// jetzt überprüfen, ob sich die Topologie im Vergleich zum ersten inMesh geändert hat 
						// - wenn nicht, dann die vtxPositionen des inMeshes aufs outMesh übertragen und dann normal mit Slide weitermachen
					
						//Absturz nach ein paar durchläufen aus mir eunbekanntem Grund
						if(initialVtxCount == inMeshFn.numVertices())
						{
							// Topologie mit großer wahrschenlichkeit unverändert, also die VtxPositionen übernehmen, die sich jetzt wahrschenlich geändert haben
							MPointArray points;
						...
						// jetzte die aktualisierten Daten für softTransformationEngine holen
						softTransformationEngine::extractStaticData(plug, data);
				...
				//es wird generell nach der creation erstmal slide ausgefÃ¼hrt, damit auch along normal und andere parameter stimmen nach ladeVorgang


				//jetzt Daten, die slide erzeugen, extrahieren und mit gespeicherten werten vergleichen.
				//wenn abweichung, wurde also nur einer dieser werte verÃ¤ndert und slide muss erfolgen
